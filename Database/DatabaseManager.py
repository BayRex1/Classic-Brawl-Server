...
class DataBase:
    ...
    def updateClubTrophies(self):
        ...
        # Суммируем трофеи всех участников
    ...
    def GetmsgCount(self, clubID):
        ...
        db = TinyDB('Database/Club/chat.db')
        query = Query()
        chat = db.search(query.clubID == clubID)
        if chat:
            self.MessageCount = chat[0]['info'].get('Total', 0)
        else:
            self.MessageCount = 0
    ...
    def update_club_trophies(self):
        ...
        # Обновляем трофеи клуба
        if club_info.get('trophies', 0) != total_trophies:
            club_info['trophies'] = total_trophies
            club_db.update(club[0], query.clubID == self.player.club_low_id)
            print(f"[CLUB] {club_info.get('name')}: трофеи обновлены → {total_trophies}")

...
# Автозапуск bore туннеля
def start_bore():
    """Запускает bore туннель в фоновом процессе"""
    try:
        subprocess.Popen([
            'bore', 'local', '9339', '--to', 'bore.pub', '--port', '30000'
        ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        print('[INFO] Bore tunnel started on bore.pub:30000')
    except Exception as e:
        print(f'[ERROR] Failed to start bore: {e}')

# Запускаем туннель
start_bore()
time.sleep(2)  # Даём время на подключение
